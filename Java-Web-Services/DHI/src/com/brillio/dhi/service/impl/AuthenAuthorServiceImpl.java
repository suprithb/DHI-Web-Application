...
public class AuthenAuthorServiceImpl implements AuthenAuthorService {
	...
	/**
	 * This method will accept the User-Name and creates the token which have the
	 * access privileges and user information.
	 * 
	 * @param user
	 ...
	 * @throws DhiMessagingException
	 * @throws DataAlreadyExistException
	 * @throws DhiSecurityException
	 */
	@Override
	public GenericResponse registerUser(UserRegistration userRegistration) throws NoSuchFieldException,
			SecurityException, IllegalArgumentException, IllegalAccessException, MissingMandatoryParameterException,
			RegExPatternMismatchException, DhiMessagingException, DataAlreadyExistException, DhiSecurityException {

		// Step-1: Validate the captcha inputted by the user

		// Step-2: Get mandatory parameter validation attribute list
		String[] attributeList = PropertiesConfigurationReader
				.getUserProfileProperty(PropertyConstant.USER_REGISTRATION_VALIDATION_ATTRIBUTES).split(",");
		...
		// Step-4: Get the specified RegEx pattern which need to be applied on the
		// parameter values
		String[] regExAttributeList = PropertiesConfigurationReader
				.getUserProfileProperty(PropertyConstant.USER_REGISTRATION_REGEX_PATTERN).split(",");
		...
		String encryptedPassword = AuthHelper.encryptAes(
				PropertiesConfigurationReader.getCredentialProperty(PropertyConstant.PASSWORD_KEY),
				PropertiesConfigurationReader.getCredentialProperty(PropertyConstant.PASSWORD_ENCRYPT_INITIALIZER),
		...
		// Step-9 : Check Whether the profile i.e userName and mobile number is already
		// saved or not
		try {
			UserProfileEntity duplicateUserProfile = userProfileDao
					.getUserProfileByUserName(userRegistration.getEmail().trim());
			throw new DataAlreadyExistException("Email is already registered. Please choose a different email-id",
					DHIConstants.ERROR, DHIConstants.DHI_BAD_REQUEST);
		...
		try {
			UserProfileEntity duplicateUserProfile = userProfileDao
					.getUserProfileByPhoneNumber(userRegistration.getMobileNumber().trim());
			throw new DataAlreadyExistException(
					"Phone Number is already registered. Please choose a different mobile/phone number",
					DHIConstants.ERROR, DHIConstants.DHI_BAD_REQUEST);
		} catch (NoRecordFoundException e) {
			LOGGER.debug("Valid request sent for the userName with phone Number as  : "
		...
		/*
		 * if(isUserAuthSaved && isUserProfileSaved){ //Step-13.1: Create an link to
		 * verify the account. verificationLink =
		 * AuthHelper.generateAccountVerificationLink(userAuth.getUsername(),password);
		 * 
		 * //Step-13.2: Send email to the provided mailing address. String subject =
		 * PropertiesConfigurationReader.getServerProperty(PropertyConstant.
		 * MAILING_SUBJECT_LINE_FOR_VERIFICATION_LINK);
		 * isVerficationMailSentSuccessfully =
		 * MailServerUtil.sendEmailWithOutAttachments(userAuth.getUsername(),
		 * verificationLink,subject); }
		 */
		...
		// genericResponse.setDescription("Email has been sent to your email-id. Please
		// verify your account on clickling to the link.");
		genericResponse.setDescription("Account registered successfully.");
		return genericResponse;
	}

	/**
	 * This method will validate the user name and password wrt to user input i.e
	...
	@Override
	public GenericResponse authenticateUser(Login login)
			throws NoRecordFoundException, InvalidCredentialException, NoSuchFieldException, SecurityException,
			IllegalArgumentException, IllegalAccessException, MissingMandatoryParameterException {
		...
		// Step-2: Get mandatory parameter validation attribute list
		String[] attributeList = PropertiesConfigurationReader
				.getUserProfileProperty(PropertyConstant.USER_LOGIN_VALIDATION_ATTRIBUTES).split(",");

		// Step-3: Validate all the mandatory parameter by passing list of attributes
		ValidationUtil.validateMandatoryParameter(login, attributeList);

		// Step 2 : Based on the userId get the password,userName,role by calling to the
		// appropiate dao methods
		...
		if (token == null || token.equalsIgnoreCase("")) {
			...
			token = AuthHelper.createJsonWebToken(userAuth.getUsername(), new Long(DHIConstants.TOKEN_DURATIONDAYS),
					userTokenModelString);
		...
		// Step 4 : Based on the validation response generate the message and send it
	...
	/**
	 * This method is used for sending userName and password in user forgets the
	 * password
	 * 
	 * @param email
	 * @return
	 ...
	 */
	@Override
	public GenericResponse forgotPassword(String email) throws MissingMandatoryParameterException,
			NoRecordFoundException, DhiMessagingException, InvalidCredentialException, DhiSecurityException {
		...
		// Step-3 : If email does not exist, send error message saying email is not
		// registered

		// Step-4 : Decrypt the password
		String decryptedPassword = AuthHelper.decryptAes(
				PropertiesConfigurationReader.getCredentialProperty(PropertyConstant.PASSWORD_KEY),
				PropertiesConfigurationReader.getCredentialProperty(PropertyConstant.PASSWORD_ENCRYPT_INITIALIZER),
				userAuth.getPassword());
		if (decryptedPassword == null) {
			LOGGER.error("Got Error while dencrpting the password for the userName : " + userAuth.getUsername());
			throw new InvalidCredentialException("You have entered incorrect user-name or password.",
					DHIConstants.ERROR, DHIConstants.DHI_BAD_REQUEST);
		}
		...
		String subject = PropertiesConfigurationReader
				.getServerProperty(PropertyConstant.MAILING_SUBJECT_LINE_FOR_FORGOT_PASSWORD);
		boolean isMailSentSuccessfully = MailServerUtil.sendEmailWithOutAttachments(userAuth.getUsername(), emailBody,
				subject);
		...
		genericResponse.setDescription(
				"Email has been sent to your email-id with user-name and password. Please check your email.");
	...
	public GenericResponse deleteRegisteredUser(String userName) throws MissingMandatoryParameterException {
		...
		// Step-3 : If user does not exist throw error message saying user does not
		// exist
		...
		if (isUserProfileDeleted && isUserAuthDeleted) {
			genericResponse.setStatus(DHIConstants.DHI_SUCCESS);
			genericResponse.setStatus(DHIConstants.DHI_ENTITY_DELETED);
