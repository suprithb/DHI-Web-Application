...
#spacy pos tagger
def get_pos_tag(us_input):
    start=time.time()
    core_pos_tagger=[]
    us_input=spell_chek(us_input)
    doc=nlp(us_input)
    for token in doc:
        Element = (token.text, token.tag_)
        core_pos_tagger.append(Element)
    end = time.time()
    time_taken = end - start
    print('Time_get_pos_tag: ',time_taken)
...
# core_ner_tagger=[]
#NER tagger spacy
def get_ner_tag(us_input):
    us_input_corrected=spell_chek(us_input)
#     print(us_input_corrected)
    doc = nlp(us_input_corrected)
#     print(doc.ents)
    core_ner_tagger=[]
    ner_tags=[]
    for ent in doc.ents:
        Elements = (ent.text, ent.label_)
        print(Elements)
        core_ner_tagger.append(Elements)
    print(core_ner_tagger)
    return core_ner_tagger

#get_ner_tag("show me sales by region australia for last week ")
...
aggregate_lexicon = ['sum','total','by','amount','summation','volume','count','average','mean']
operator_lexicon = ['less than','greater than','more than','lesser than','lower than','higher than','above','equal to','equals','as','is']
...
having_lexicon=['having','with','where']
...
#We use pos tags to eliminate useless words form the query to get the header of the graph
def get_graph_header(us_input):
    start=time.time()
    us_input_corrected=spell(us_input)
    print(us_input_corrected)
    doc = nlp(us_input_corrected)
    core_tokens=[]
    for ent in doc:
        core_tokens.append(ent.text)
    print(core_tokens)
    df_new = pd.DataFrame({'ngram': core_tokens})
    pos_tag = [b for a,b in get_pos_tag(us_input_corrected)]
...
# get_graph_header('show me sales by rgion australia')
...
#     core_tokens = list(parser.tokenize(us_input))
    core_tokens = nltk.word_tokenize(us_input)
    ...
    pos_tag = [b for a,b in get_pos_tag(us_input)]
    print(get_ner_tag(us_input))
    ner_tag = [b for a,b in get_ner_tag(us_input)]
...
#     df_final = pd.concat([df2,df3],axis=1)
    ...
    Date_Regex = re.compile(r"^(0*[1-9]|[12][0-9]|3[01])[- \/.]((0*[1-9]|1[012])|January|Jan|February|Feb|March|Mar|April|Apr|May|May|June|Jun|July|Jul|August|Aug|September|Sep|October|Oct|November|Nov|December|Dec)[- \/.]\d{2,4}$")
    us_input = us_input.split()
#     print(us_input)
    matches =[]
    for element in us_input:
#         print(element)
        match = Date_Regex.match(element)
...
#         print(match[0])
    ...
    for i in range(len(dates)):        
        Dates.append(datetime.strptime(dates[i],'%d/%m/%Y'))
#         Date_2=datetime.strptime(dates[1],'%d/%m/%Y')
    print(Dates)
    if len(Dates) ==0:
        date_present = 'False'
    elif len(Dates) ==1:
    ...
    time_taken = end - start
#     print('Time_build_Dataframe: ',time_taken)
    return df_final, df_metadata,df_clean_data_transposed, current_date, previous_date, date_present,us_input
# build_Dataframe(us_input,data_file_loc)


# In[92]:


#us_input= "show me sales by region australia last week"
#data_file_loc='C:\Dhi\sample_data_ashish.csv'
#df_fin, df_met,df_cl,cu_date,pre_date,dat_pr,us_input = build_Dataframe(us_input,data_file_loc)
#print(df_fin)
#Bellow is the o/p of build dataframe
...
def build_info(df_final,df_metadata,current_date,previous_date):
    ...
    if 'True' in aggregate_present:
        ...
        sum_lex = ['sum','total','summation','volume','amount','by']
        count_lex = ['count']
...
#this us the main function after we have the query defined as other-other
#us_input- is the query asked
#data_file_loc - the data file
#col_alias_user - the alias file
...
model = load_model('C:/Server/DHI/dhi_intelligence/AI_V3/model.h24')
...
intent, unique_intent, sentences = load_dataset("C:/Server/DHI/dhi_intelligence/AI_V3/nl_model_data_1.csv")
...
#thus cleans the data like removing plurals etc
...
#This is the main function that we call form ui
#This gives us a graph,query o/p, the sql query.We have query as an o/p so that wahen we refresh we have the query
def build_response(us_input,usr_name,data_file_loc,alias_file_loc):
    print('Inside the build response')
    try:
        ...
        #it will append what is incorrect to the user input
        us_input = spell_chek(us_input)
        ...
        if intent == 'greet_hello':
            ...
            print('DF value : ', out)
            #Added image paths
            image_path='D:/Brillio_Stuffs/Python_AI/graph_output\\test_user_bar_chart_hz_2019_12_02_12_09_10.png'
            ...
            #print(query)
            print('Response from build_response : ', out)
...
#def build_response(us_input,data_file_loc,alias_file_loc):
#    try:
#        us_input =  us_input.lower()
#        query = ''
#        us_input = spell_chek(us_input)
...
#            print(graph_header)
#            us_input,col_alias_user,col_dict = get_alias(us_input,alias_file_loc)
#            if (col_alias_user == None) | (col_dict == None):
#                out = ''
#                graph_header = ''
#                query = ''
#                return out, graph_header, query
...
#data_file_loc = ('C:\\Dhi\\sample_data_ashish.csv')
#alias_file_loc = ('C:\\Dhi\\sample_data_ashish_alias.csv')
